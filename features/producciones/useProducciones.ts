 ...
 * produccion.service.ts a la UI. Misma técnica de "una fila de más" que
...
import type { ProduccionListada } from './produccion.service';
...
export function useProducciones() {
  ...
  const cargar = useCallback(async (offset: number, reemplazar: boolean) => {
    ...
    setError(null);
    try {
      const pagina = await produccionService.listarProducciones(PAGE_SIZE + 1, offset);
  ...
  const producirLote = useCallback(
    async (input: ProducirLoteInput) => {
      const resultado = await produccionService.producirLote(input);
      await cargar(0, true);
      return resultado;
    },
    [cargar]
  ...
  const registrarVentaYMerma = useCallback(
    async (produccionId: number, input: VentaMermaInput) => {
      await produccionService.registrarVentaYMerma(produccionId, input);
      await cargar(0, true);
    },
    [cargar]
  );
  ...
  const anularProduccion = useCallback(
    async (produccionId: number) => {
      await produccionService.anularProduccion(produccionId);
      await cargar(0, true);
    },
    [cargar]
  );
  ...
  return {
    ...
    anularProduccion,
    obtenerProduccion: produccionService.obtenerProduccion,
  };
}
